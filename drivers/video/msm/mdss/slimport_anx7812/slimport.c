...
struct anx7816_platform_data
{
	...
	struct regulator *reg_8941_s3a;
	struct regulator *reg_8941_l18;
	spinlock_t lock;
};
...
static int slimport7816_avdd_power(unsigned int onoff)
{
	...
	if(onoff)
	{
		rc = regulator_enable(pdata->reg_8941_s3a);
		if(rc < 0)
		{
			pr_err("%s %s: failed to enable reg_8941_s3a regulator rc=%d\n",
				   LOG_TAG, __func__, rc);
		}
		rc = regulator_enable(pdata->reg_8941_l18);
		if(rc < 0)
		{
			pr_err("%s %s: failed to enable reg_8941_l18 regulator rc=%d\n",
				   LOG_TAG, __func__, rc);
		}
		/* Enable 1.0V LDO */
	...
	{
		rc = regulator_disable(pdata->reg_8941_s3a);
		if(rc < 0)
		{
			pr_err("%s %s: failed to enable reg_8941_s3a regulator rc=%d\n",
				   LOG_TAG, __func__, rc);
		}
		rc = regulator_disable(pdata->reg_8941_l18);
		if(rc < 0)
		{
			pr_err("%s %s: failed to enable reg_8941_l18 regulator rc=%d\n",
				   LOG_TAG, __func__, rc);
...
/* To do : regulator control after H/W change */
	...
	if (!reg_8941_s3a) {
		...
		pdata->reg_8941_s3a = reg_8941_s3a;
		#if 0
	...
	if (!reg_8941_l18) {
		...
		pdata->reg_8941_l18 = reg_8941_l18;
		#if 0
	...
	struct device *dev, struct anx7816_platform_data *pdata)
{
	...
	pdata->gpio_p_dwn = of_get_named_gpio_flags(
		np, "anx7808-pdown-gpio", 0, NULL);

	pdata->gpio_reset = of_get_named_gpio_flags(
		np, "anx7808-reset-gpio", 0, NULL);

	pdata->gpio_cbl_det = of_get_named_gpio_flags(
		np, "anx7808-cbl-gpio", 0, NULL);
	pr_info(KERN_INFO
	...
	if (pdata->external_ldo_control) {
		pdata->gpio_v10_ctrl = of_get_named_gpio_flags(
			np, "anx7808-v10-gpio", 0, NULL);
	pr_info(KERN_INFO "%s gpio_v10_ctrl %d avdd33-en-gpio %d\n",
...
bool is_slimport_hdmi(void)
{
	return (sp_tx_cur_cable_type() == DWN_STRM_IS_HDMI_7730) ? TRUE : FALSE;
}
EXPORT_SYMBOL(is_slimport_hdmi);
