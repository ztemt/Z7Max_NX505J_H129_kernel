...
static void mdss_dsi_panel_bklt_dcs(struct mdss_dsi_ctrl_pdata *ctrl, int level)
{
	...
	if (level < 6)		//The least PWM should greater than 2%
		level = 6;
...
#elif defined CONFIG_ZTEMT_MIPI_1080P_R63311_SHARP_IPS_5P0_NX503A

  if (level < 0)
		level = 0;
  
  if(level > MAX_LCD_BL_VAL) 
      level = MAX_LCD_BL_VAL; //4095

  if((level < ZTE_LCD_BL_FIX_MAX_VAL)&& (level> ZTE_LCD_BL_FIX_MIN_VAL))
     level = level - ZTE_LCD_BL_FIX_STEP; 
...
#ifdef CONFIG_ZTEMT_LCD_BACKLIGHT
      if(IsBgvalueTranEnable())
      {
          bl_level = ZteBgvalue2pwm(bl_level);
...
#ifdef CONFIG_ZTEMT_MIPI_1080P_R63311_SHARP_IPS_5P0_NX507J
	bl_level = ztemt_pwm_adjust_r63311_sharp_ips_5p0_nx507j(bl_level);
    if(bl_level > ctrl_pdata->bklt_max)
        bl_level = ctrl_pdata->bklt_max;
    #ifdef CONFIG_ZTEMT_LCD_DEBUG_EN
    printk(KERN_ERR"-----[%s] after adjust bl_level = %d\n", __func__, bl_level);
    #endif
#endif

	switch (ctrl_pdata->bklt_ctrl) {
	case BL_WLED:
		pr_debug("%s: BL_WLED level = %d\n", __func__, bl_level);
